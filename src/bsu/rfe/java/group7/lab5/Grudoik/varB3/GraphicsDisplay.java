package bsu.rfe.java.group7.lab5.Grudoik.varB3;
...
public class GraphicsDisplay extends JPanel {
    ...
    public GraphicsDisplay() {
        ...
        graphicsStroke = new BasicStroke(2.0f, BasicStroke.CAP_BUTT,
                BasicStroke.JOIN_ROUND, 10.0f, new float[] {6,3,6,3,6,3,1,3,1,3,1,3}, 0.0f);
    ...
    protected boolean functionInRange(double[] point) {
        int intFuncValue = (int) point[1];
        int even = intFuncValue % 2;
...
// Выбрать синий цвета для контуров маркеров
        canvas.setColor(Color.BLUE);
// Выбрать синий цвет для закрашивания маркеров внутри
...
// Шаг 2 - Организовать цикл по всем точкам графика
        for (double[] point: graphicsData) {
            ...
            cross.moveTo(center.x, center.y+5.5);
            cross.lineTo(center.x, center.y -5.5);
            cross.moveTo(center.x+5.5, center.y);
            cross.lineTo(center.x-5.5, center.y);
            marker.setFrameFromCenter(center, corner);
            canvas.draw(marker); // Начертить контур маркера
            canvas.draw(cross); // Начертить крест внутри маркера
        }
